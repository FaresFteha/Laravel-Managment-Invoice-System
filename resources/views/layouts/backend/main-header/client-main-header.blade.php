...
<ul class="navbar-nav navbar-nav-icons ms-auto flex-row align-items-center">
    ...
    <li class="nav-item dropdown"><a class="nav-link pe-0" id="navbarDropdownUser" href="#" role="button"
            ...
            <?php
            $profile = App\Models\Client::firstwhere('id', Auth::guard('client')->user()->id);
            ?>
            @if ($profile->photo)
                <div class="avatar avatar-xl">
                    <img class="rounded-circle"
                        src="{{ asset('storage/Attachments/Client-Attachments/' . $profile->photo) }}" alt="" />
                </div>
            @else
                <div class="avatar avatar-xl">
                    <img class="rounded-circle" src="{{ asset('asset/backend/src/img/team/avatar.png') }}"
                        alt="" />
                </div>
            @endif
        ...
        <div class="dropdown-menu dropdown-menu-end py-0" aria-labelledby="navbarDropdownUser">
            <div class="bg-white dark__bg-1000 rounded-2 py-2">
                <a class="dropdown-item fw-bold text-warning" href="#!"><span
                        class="fas fa-crown me-1"></span><span>{{ Auth::guard('client')->user()->first_name }}</span></a>

                <div class="dropdown-divider"></div>
                <a class="dropdown-item" href="{{ route('incoices.client.profile') }}">الحساب &amp; الاعدادت</a>
